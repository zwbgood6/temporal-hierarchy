...
# modified from: https://github.com/alexlee-gk/video_prediction

class HTML:
    def __init__(self, web_dir, title, reflesh=0, sub_dir=None):
        self.title = title
        self.web_dir = web_dir
        if sub_dir is not None:     # parent HTML without own data folders
            self.img_dir = os.path.join(os.path.join(self.web_dir, sub_dir), 'images')
            self.rel_img_str = os.path.join(sub_dir, "images")
        else:
            self.img_dir = os.path.join(self.web_dir, 'images')
            self.rel_img_str = "images"
                    ...
                    with td(style="word-wrap: break-word;", halign="center", valign="top", **colspan):
                        with p():
                            if im is not None and link is not None:
                                with a(href=os.path.join(self.rel_img_str, link)):
                                    img(style=image_style, src=os.path.join(self.rel_img_str, im))
